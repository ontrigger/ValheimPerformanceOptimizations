...
public static class PrefabPatches
{
	...
	private static int PatchLocalParticleSystemCulling(GameObject prefab)
	{
		...
		foreach (var particleSystem in prefab.GetComponentsInChildren<ParticleSystem>(true))
		{
			...
			var cullingMode = particleSystem.gameObject.name switch
			{
				...
				"flames (1)" => ParticleSystemCullingMode.PauseAndCatchup,
				"embers" => ParticleSystemCullingMode.PauseAndCatchup,
	...
	private static string GetPrefabParticlePath(Transform particleTransform)
	{
		var path = particleTransform.name;
		var currentTransform = particleTransform;
	...
	private static void Postfix(ZNetScene __instance, Dictionary<int, GameObject> ___m_namedPrefabs)
	{
		...
		foreach (var prefab in namedPrefabs.Values)
		{
			...
			particleCullingPatched += PatchLocalParticleSystemCulling(prefab);
			patched += 1;
		}
		if (SystemInfo.graphicsDeviceType != GraphicsDeviceType.Null)
		{
			...
			VPO.Logger.LogInfo("Combined prefab mats in "
		...
		VPO.Logger.LogInfo(
			$"Updated culling for {particleCullingPatched} local-space flare or flame particle systems"
		);
		VPO.Logger.LogInfo($"Patched {patched} prefabs");
